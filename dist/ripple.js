...
export function ripple(selector, _options = {}) {
    const sharedOptions = Object.assign(Object.assign({}, defaults), _options);
    ...
    const trigger = function (event, sharedOptions) {
        var _a;
        let rippleElement;
        // Get data-* from the element
        const dataset = Object.fromEntries(Object.entries(this.dataset).map(([key, value]) => {
            const _key = key;
        ...
        const options = Object.assign(Object.assign({}, sharedOptions), dataset);
        ...
        if (options.multi ||
            ...
            const color = options.color == 'auto' ? getComputedStyle(this).color : options.color;
            const css = {
        ...
        if (!options.multi) {
            ...
            rippleElement = (_a = this.querySelector('.ripple')) !== null && _a !== void 0 ? _a : undefined;
        }
        if (rippleElement) {
            // Kill animation
            ...
            if (options.multi) {
                ...
                'animationend webkitAnimationEnd oanimationend MSAnimationEnd'
                    ...
                    rippleElement &&
                        rippleElement.addEventListener(animationEndEvent, () => {
    ...
    document.querySelectorAll(selector).forEach((element) => {
        if (element instanceof HTMLElement) {
            element.addEventListener(sharedOptions.on, (event) => trigger.call(element, event, sharedOptions));
        }
    });
    return trigger;
}
